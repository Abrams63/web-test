...
var userAgent = navigator.userAgent.toLowerCase(),
  ...
  loaderTimeoutId,
	...
	plugins = {
		...
		captcha: $(".recaptcha"), // reCAPTCHA elements
		search: $(".rd-search"),
  ...
  if( plugins.preloader.length ){
	...
	function isValidated(elements, captcha) {
		...
		if (elements.length) {
			...
			if (captcha) {
				if (captcha.length) {
					// Only validate reCAPTCHA if grecaptcha is loaded
					if (typeof grecaptcha !== 'undefined') {
						return validateReCaptcha(captcha) && errors == 0;
					} else {
						// If grecaptcha is not loaded, show error and return false
						captcha
							.siblings('.form-validation')
							.html('reCAPTCHA is not loaded properly.')
							.addClass('active');
						captcha
							.closest('.form-group')
							.addClass('has-error');
	...
	function validateReCaptcha(captcha) {
		// Check if reCAPTCHA is on the page and has been completed
		...
		var checkRecaptcha = function() {
			...
			if (response && response.length > 0) {
				// Valid reCAPTCHA response exists
				captcha
					.closest('.form-group')
				...
				// Max attempts reached, reCAPTCHA not completed
				captcha
					.siblings('.form-validation')
				...
				// Set up event to detect when reCAPTCHA is completed
				var checkRecaptchaCompletion = setInterval(function() {
					var newResponse = grecaptcha.getResponse();
					if (newResponse && newResponse.length > 0) {
						clearInterval(checkRecaptchaCompletion);
						captcha
							.closest('.form-group')
							.removeClass('has-error');
						captcha
	...
	// Wait a bit to ensure grecaptcha is fully loaded before trying to initialize
		if (typeof grecaptcha === 'undefined') {
			console.error('reCAPTCHA library is not loaded');
			return;
		}
		
		// First get the reCAPTCHA configuration from the server
		$.get('/api/recaptcha/config', function(configData) {
			var siteKey = configData.site_key || '6LcB0RUsAAAAANmYJjeuJOrzRm62JzFiaHjINw-g'; // Fallback to default key if not configured properly
			
			// Use a small delay to ensure reCAPTCHA is ready
			setTimeout(function() {
				if (typeof grecaptcha === 'undefined') {
					console.error('reCAPTCHA library is not loaded after timeout');
					return;
				}
				...
				for (i = 0; i < plugins.captcha.length; i++) {
					...
					// Ensure the captcha item has an ID
					if (!$capthcaItem.attr('id')) {
						$capthcaItem.attr('id', 'recaptcha-' + i);
					}

					grecaptcha.render(
						$capthcaItem.attr('id'),
						{
							sitekey: $capthcaItem.attr('data-sitekey') || siteKey,
							...
							'expired-callback': function() {
								// Reset the form validation when reCAPTCHA expires
								$capthcaItem.siblings('.form-validation').html('').removeClass('active');
								$capthcaItem.closest('.form-group').removeClass('has-error');
							}
						}
					);
				...
				// Initialize reCAPTCHA for forms that have .recaptcha class but aren't in plugins.captcha
				$('.recaptcha:not([id])').each(function(index) {
					var $this = $(this);
					if (!$this.attr('id')) {
						$this.attr('id', 'recaptcha-' + index);
					}
					
					grecaptcha.render(
						$this.attr('id'),
						{
							sitekey: $this.attr('data-sitekey') || siteKey, // Use server-provided key as fallback
							callback: function (e) {
								$this.trigger('propertychange');
							},
							'expired-callback': function() {
								// Reset the form validation when reCAPTCHA expires
								$this.siblings('.form-validation').html('').removeClass('active');
								$this.closest('.form-group').removeClass('has-error');
							}
						}
					);
					$this.after("<span class='form-validation'></span>");
				});
			}, 500); // Wait 500ms before initializing
	}).fail(function() {
			console.error('Failed to load reCAPTCHA configuration from server. Using default key.');
			
			// Fallback to default behavior if server config fails
			setTimeout(function() {
				if (typeof grecaptcha === 'undefined') {
					console.error('reCAPTCHA library is not loaded after timeout');
					return;
				...
				for (i = 0; i < plugins.captcha.length; i++) {
					...
					grecaptcha.render(
						$capthcaItem.attr('id'),
						{
							sitekey: $capthcaItem.attr('data-sitekey') || '6LcB0RUsAAAAANmYJjeuJOrzRm62JzFiaHjINw-g',
							size: $capthcaItem.attr('data-size') ? $capthcaItem.attr('data-size') : 'normal',
							theme: $capthcaItem.attr('data-theme') ? $capthcaItem.attr('data-theme') : 'light',
							callback: function (e) {
								$('.recaptcha').trigger('propertychange');
				...
				$('.recaptcha:not([id])').each(function(index) {
					...
					grecaptcha.render(
						$this.attr('id'),
						{
							sitekey: $this.attr('data-sitekey') || '6LcB0RUsAAAAANmYJjeuJOrzRm62JzFiaHjINw-g',
							callback: function (e) {
								$this.trigger('propertychange');
							},
							'expired-callback': function() {
			...
			$form.attr('novalidate', 'novalidate').ajaxForm({
				...
				beforeSubmit: function (arr, $form, options) {
					...
					if (isValidated(inputs, captcha)) {
						...
						// veify reCaptcha
						if (captcha.length) {
							// Check if grecaptcha is loaded before getting response
							if (typeof grecaptcha !== 'undefined') {
								...
								var captchaMsg = {
									'CPT002': 'reCAPTCHA is not loaded properly'
								};
				...
				success: function (result) {
					...
					if (result === "MF000") {
						...
						if (output.hasClass("snackbars")) {
							output.html(' <p class="snackbars-left"><span class="icon icon-xxs mdi-alert-outline text-middle"></span><span>' + msg[result] + '</span></p>');
						} else {
